...
const Product = ({ images, id, thumbnail }) => {
  ...
  const wishFind = wishlist.find((wish) => wish.id === id);

  return (
    <div className='border rounded-md h-[400px] w-[300px] p-5 relative mb-10'>
      <div className='absolute z-10 top-3 left-3 bg-[#fff] px-3 py-3 rounded-full shadow-lg cursor-pointer'>
        {wishFind ? (
          <AiFillHeart
      ...
      >
        <div className='max-w-full w-auto rounded-lg h-[250px] overflow-hidden'>
          <img
            src={thumbnail}
            alt='images'
            className=' duration-500 mx-auto  w-full  h-[250px]  object-cover mb-5 select-none rounded-lg group-hover:scale-105'
          />
        </div>
      </Link>
